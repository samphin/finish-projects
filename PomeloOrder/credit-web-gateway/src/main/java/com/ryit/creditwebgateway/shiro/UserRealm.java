...
//        }
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        if (null == user.getAdminFlag() || !user.getAdminFlag().equals(1)) {
            throw new AuthenticationException("无权限登录");
        }
        // 开始认证，要AccessToken认证通过，且Redis中存在RefreshToken，且两个Token时间戳一致
        if (JwtUtil.verify(token) && redisUtil.hasKey(String.format(RedisConstants.PREFIX_CREDIT_WEB_REFRESH_TOKEN, account))) {
            // 获取RefreshToken的时间戳
