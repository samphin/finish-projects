...
//        }
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        // 查询用户是否存在
        CreditUser user = (CreditUser) redisUtil.getObject(String.format(RedisConstants.PREFIX_CREDIT_USER, account));
        if (null == user) {
            user = creditUserFeignClient.getCreditUserByPhone(account);
        }
        if (user == null) {
            throw new AuthenticationException("该帐号不存在(The account does not exist.)");
        }
        // 开始认证，要AccessToken认证通过，且Redis中存在RefreshToken，且两个Token时间戳一致
        if (JwtUtil.verify(token) && redisUtil.hasKey(String.format(RedisConstants.PREFIX_CREDIT_API_REFRESH_TOKEN, account))) {
            // 获取RefreshToken的时间戳
            String currentTimeMillisRedis = redisUtil.get(String.format(RedisConstants.PREFIX_CREDIT_API_REFRESH_TOKEN, account));
            ...
            if (JwtUtil.getClaim(token, JwtConstant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                RequestContext.getCurrentContext().addZuulRequestHeader("authFlag", user.getAuthFlag().toString());
                //创建用户缓存
                redisUtil.set(String.format(RedisConstants.PREFIX_CREDIT_USER, account), user,
