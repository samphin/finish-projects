...
public class LoginController {
    ...
    public ResponseData login(@Validated(LoginDto.Login.class) @RequestBody LoginDto requestData, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (null == user) {
            user = sysUserService.queryLoginInfoByPhone(phone);
        }
        ...
        //查询用户角色权限信息
        List<SysSimpleRoleVo> roleVoList = sysUserService.queryUserHaveRoles(user.getId());
    ...
    public ResponseData simpleRegister(@RequestBody @Validated RegisterDto registerDto) {
        ...
        return bl ? ResponseData.success() : ResponseData.failure();
    }
    ...
    public ResponseData register(@Validated(LoginDto.Register.class) @RequestBody LoginDto registerDto, HttpServletRequest request) {
        ...
        //校验手机中是否已注册
        boolean b = sysUserService.existTelephone(phone);
        if (b) {
