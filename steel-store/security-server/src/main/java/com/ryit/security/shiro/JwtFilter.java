...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (redisUtil.hasKey(String.format(RedisConstants.PREFIX_API_REFRESH_TOKEN, account))) {
            ...
            if (JwtUtil.getClaim(token, JwtConstant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                redisUtil.set(String.format(RedisConstants.PREFIX_API_REFRESH_TOKEN, account), currentTimeMillis,
                ...
                httpServletResponse.setHeader(JwtConstant.AUTHORIZATION, token);
                httpServletResponse.setHeader("Access-Control-Expose-Headers", JwtConstant.AUTHORIZATION);
                return true;
            }
        }
        return false;
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws IOException {
        ...
        ResponseData result = ResponseData.failure(SystemErrorEnum.PLEASE_LOGIN);
    ...
    private void response401(ServletResponse resp) {
        try {
            ...
            httpServletResponse.sendRedirect("/admin/unauth");
        } catch (IOException e) {
